...
export class QueueService {
  ...
  public enqueue(payload: NotificationPayload): number {
  ...
  public claimAvailable(): QueueJob[] {
    return this.repository.claimAvailable(env.WORKER_BATCH_SIZE, this.workerId);
  }

  public complete(job: QueueJob): void {
    this.repository.markCompleted(job.id);
  }

  public retry(job: QueueJob, errorMessage: string, retryAfterMs?: number): void {
    const delayMs = retryAfterMs ?? this.computeBackoffMs(job.attempts + 1);
    const availableAt = new Date(Date.now() + delayMs).toISOString();
    this.repository.markRetry(job, errorMessage, availableAt);
  }

  public fail(job: QueueJob, errorMessage: string): void {
    this.repository.markFailed(job, errorMessage);
  }
